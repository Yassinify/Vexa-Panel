...
const derivedKey = crypto.pbkdf2Sync(password, Buffer.from(salt, 'hex'), 100000, 32, 'sha256').toString('hex');
...
* `GET /sub/:profileId`: Public subscription download link (returns Base64-encoded URI list).
  * *Query Parameter:* `?canonical=1` (Optional) — Normalizes and re-serializes nodes through canonical URI generation.
